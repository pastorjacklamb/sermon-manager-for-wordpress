...
class Tax_Meta_Class {
	...
	public function load_scripts_styles() {
		...
		if (in_array($taxnow,$this->_meta_box['pages'])){
			...
			wp_enqueue_style( 'tax-meta-clss', plugins_url('/css/Tax-meta-class.css', __FILE__) );
			// Enqueue Meta Box Scripts
			wp_enqueue_script( 'tax-meta-clss', plugins_url('/js/tax-meta-clss.js', __FILE__), array( 'jquery' ), null, true );
	...
	*/
	...
	public function show_field_repeater( $field, $meta ) {
   			...
   			foreach ($meta as $me){
				...
				<span class="at-re-toggle"><img src="';
   				if ($this->_Local_images){
   					echo $plugin.'/includes/tax-meta-class/images/edit.png';
   				}else{
				...
				<img src="';
				if ($this->_Local_images){
					echo $plugin.'/includes/tax-meta-class/images/remove.png';
				}else{
		...
		echo '<img src="';
		if ($this->_Local_images){
			echo $plugin.'/includes/tax-meta-class/images/add.png';
		}else{
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
	...
	public function show_field_image( $field, $meta ) {
		...
		if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
			$html .= "<span class='mupload_img_holder'><img src='".$meta['src']."' /></span><br />";
